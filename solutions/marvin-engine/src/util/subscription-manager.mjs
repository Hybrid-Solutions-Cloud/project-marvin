...
function defaultMicrosoftResource(calendar) {
  const providerCalendarId = normalizeString(calendar?.providerCalendarId);
  return providerCalendarId ? `/me/calendars/${providerCalendarId}/events` : "/me/calendar/events";
}

function defaultProviderResource(providerId, calendar) {
  return providerId === "microsoft" ? defaultMicrosoftResource(calendar) : "";
}
  ...
  for (const calendar of calendars) {
    ...
    if (!notificationUrl) {
      nextRecords.push(upsertErrorRecord(existing, {
        calendarId: calendar.id,
        provider: providerId,
        resource: normalizeString(existing?.resource || defaultProviderResource(providerId, calendar)),
        notificationUrl: "",
        status: "skipped",
        lastCheckedAt: new Date(nowMs).toISOString(),
        updatedAt: new Date(nowMs).toISOString(),
