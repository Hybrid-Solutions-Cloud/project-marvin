...
export class GoogleCalendarAdapter {
  ...
  async listSourceEvents(calendar, options = {}) {
    ...
    return events.map((event) => ({
      ...
      calendarId: calendar.id,
      iCalUId: normalizeString(event.iCalUID),
      providerEventIdentity: normalizeString(event.iCalUID) ? `ical:${normalizeString(event.iCalUID)}` : "",
      ...
      availability: normalizeString(event.transparency).toLowerCase() === "transparent" ? "free" : "busy",
  ...
  buildGooglePayload(operation) {
    ...
    return {
      ...
      transparency: payload.availability === "free" || payload.availability === "workingElsewhere" ? "transparent" : "opaque",
