...
The Marvin UI is the product-owned front door for the Marvin account that owns the linked calendars, provider credentials, and automation state:

- creating or reviewing the Marvin account
...
- collecting provider app settings needed for live authentication
...
- keeping first-run setup and ongoing account management inside the same Marvin browser surface
...
6. Save the generated Marvin account configuration and local `.marvin` state.
...
10. Connect each provider account from Marvin's Connected Calendars area after Marvin setup has been saved.
...
13. Run `Validate Access` for the calendars that should now be connected.
14. Review the Marvin Admin card to confirm the saved account name, internal account ID, timezone, sync window, and automation state.
...
- Marvin admin summary state such as active account ID, timezone, sync window, and automation state
...
That means Marvin no longer sends users into the old Keeper login path when a provider is not actually configured, and Marvin now blocks connect attempts earlier when access settings are still missing.
...
- Marvin can keep the operator inside Marvin-owned setup and management flows instead of defaulting back to the old Keeper bridge
