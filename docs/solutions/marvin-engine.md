...
- Microsoft 365 and Outlook, Apple / CalDAV, and Google support delivered in that order
...
## What exists now on August 19, 2026

Implemented now:

- shared profile loader
- shared runtime-context loader for profile, tokens, provider secrets, adapters, and engine state
- encrypted, schema-versioned token-state loader with atomic replacement
- encrypted local provider-secret loader with automatic migration from legacy plaintext state
- schema-versioned, atomic local runtime-status store
...
- Microsoft Graph source reads, mirror writes, and stale-mirror cleanup in the mocked runtime path
- Apple / CalDAV REPORT-based source reads plus PUT and stale-mirror cleanup support in the mocked runtime path
- Google Calendar source reads, mirror writes, and stale-mirror cleanup in the mocked runtime path
...
- production-grade long-term token lifecycle controls beyond on-demand refresh and encrypted persistence
- full provider webhook/subscription renewal parity beyond the initial local Microsoft and Google lifecycle
- independent security review of the production management API boundary
...
5. Provider adapters read or refresh credentials as needed, then perform deterministic live upserts. Provider deletion is disabled unless the managed-mirrors-only safety mode is explicitly enabled.
6. Mapping state is written to disk after mock or live apply.
7. A source deletion is considered only when the provider returns an explicit tombstone, the source read succeeded, and the mapping proves Marvin ownership. Absence from a list is never treated as deletion.
8. Refreshed token state is written back into `.marvin/tokens/*.tokens.json`.
9. The optional daemon runner repeats that cycle on an interval, records runtime status in `.marvin/runtime/*.runtime.json`, maintains Microsoft and Google subscription/watch state in `.marvin/subscriptions/*.subscriptions.json` when a hosted Marvin URL is configured, and can wake early from queued webhook-triggered automation state instead of waiting only for the next blind interval.
...
## Verification maintained through August 19, 2026
...
- the Project Marvin browser surface starts with a security account, opens directly into calendar management, and begins provider authorization immediately after a calendar email is added
...
- Marvin has a local operator-journey smoke covering Entra-backed workspace bootstrap, multi-provider account save, Microsoft callback auth, Apple / CalDAV direct validation, Google pending validation, and runtime start/stop
- loopback development uses an explicit development-session endpoint; hosted deployments reject that endpoint and require Microsoft Entra authentication
...
- Marvin ignores absence-based cleanup, records explicit provider tombstones, and sends no provider deletion while the default safety mode is active; mocked cleanup proves that deliberately enabled deletion is limited to Marvin-owned mappings
- Microsoft delta cursors, immutable IDs, recurrence and exception metadata, all-day events, bounded throttling retries, deterministic create transaction IDs, and invite-safe payloads are covered by a focused contract smoke
- the daemon-style local runtime can execute a three-provider sync cycle, persist runtime status, record the last result for later inspection, and now persist webhook-trigger wake reasons for runs that started from queued Microsoft or Google notifications
- a long-running Marvin daemon can wake early from queued webhook state instead of waiting for the full configured interval
- Marvin can start, stop, and inspect the local daemon process through shared runtime-manager helpers
- Marvin can queue local automation work from hosted Microsoft and Google webhook callbacks and consume that queued wake-up state during a daemon run
...
Other repository tracks should be treated as references or interim bridges, not as Marvin's finished product boundary.
