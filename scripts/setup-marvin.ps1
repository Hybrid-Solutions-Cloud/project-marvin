...
$providerSecrets | ConvertTo-Json -Depth 6 -Compress | node .\scripts\store-marvin-provider-secrets.mjs --root $root --profile $ProfileSlug | Out-Null
if ($LASTEXITCODE -ne 0) {
  throw 'Encrypting provider secrets failed during Project Marvin setup.'
}
...
Write-Host "3. Historical reference generation is opt-in and is not part of the Project Marvin install path."
Write-Host "4. Use the Calendars page to add each account, complete provider authorization, and verify every calendar is Ready."
Write-Host "5. Review privacy rules, then start synchronization from the Dashboard."
Write-Host "6. Run npm run marvin:doctor for a repo-level health check and next verification guidance."
Write-Host "7. Run npm run marvin:smoke-operator-journey for one Project Marvin setup/auth/validation/runtime check."
Write-Host "8. Run npm run marvin:dry-run to inspect mirror planning from the saved profile."
Write-Host "9. Run npm run marvin:verify-local for the complete local Project Marvin verification flow."
