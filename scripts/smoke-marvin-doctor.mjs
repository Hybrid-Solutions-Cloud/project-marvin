...
try {
  ...
  assert.match(report.verification.localDevelopmentCommand, /MARVIN_DEV_AUTH_ENABLED/);
  ...
  assert.ok(report.nextSteps.some((step) => /Calendars page/i.test(step)));
  ...
  assert.ok(emptyReport.nextSteps.some((step) => /MARVIN_DEV_AUTH_ENABLED/i.test(step)));
