...
statusStore.save({
  ...
  recentRuns: [{
    ...
    success: true,
    sourceLoad: { loaded: 1, events: [{ id: "legacy-private-event-id", subject: "Legacy private event subject", description: "Legacy private event body" }] },
    applyResult: { attempted: 1, succeeded: 1, failed: 0, skipped: 0, results: [{ sourceEventId: "legacy-private-event-id", targetEventId: "legacy-private-target-id", status: "created" }] }
  }]
});
...
const rawRuntimeStatus = readFileSync(buildRuntimeStatusPath(tempRoot, profile.name), "utf8");
const runtimeStatus = JSON.parse(rawRuntimeStatus);
...
assert.ok(Array.isArray(runtimeStatus.recentRuns) && runtimeStatus.recentRuns.length === 2);
assert.doesNotMatch(rawRuntimeStatus, /Quarterly review|Family dinner|Doctor visit|Review meeting|Annual checkup/);
assert.doesNotMatch(rawRuntimeStatus, /Legacy private event subject|Legacy private event body|legacy-private-event-id|legacy-private-target-id/);
assert.doesNotMatch(rawRuntimeStatus, /ms-access|ms-refresh|google-access|google-refresh|apple-password/);
