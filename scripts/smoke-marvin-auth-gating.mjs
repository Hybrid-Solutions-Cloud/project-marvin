...
const server = spawn(process.execPath, ["scripts/marvin-onboard-ui.mjs"], {
  ...
  env: {
    ...
    MARVIN_DEV_AUTH_EMAIL: "auth-gating@example.com",
    MARVIN_DEV_AUTH_DISPLAY_NAME: "Auth Gating Operator",
    MARVIN_SESSION_TTL_SECONDS: "2"
  },
  stdio: "ignore"
});
...
try {
  ...
  }, 10000, "bootstrap before workspace binding");
  ...
  const login = await requestJson(`http://127.0.0.1:${port}/marvin-api/auth/dev`, { method: "POST" });
  ...
  const bootstrapAfterLogin = await requestJson(`http://127.0.0.1:${port}/marvin-api/bootstrap`);
  assert.equal(bootstrapAfterLogin.body.config.profileName, profileName);
  ...
  await sleep(2100);
  const bootstrapAfterExpiry = await requestJson(`http://127.0.0.1:${port}/marvin-api/bootstrap`);
  assert.equal(bootstrapAfterExpiry.body.authenticated, false);
  assert.equal(bootstrapAfterExpiry.body.requiresLogin, true);
