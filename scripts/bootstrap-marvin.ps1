...
try {
  ...
  if (-not $SkipSmokeSetup) {
    Write-Host 'Running Marvin setup smoke before bootstrapping...' -ForegroundColor Cyan
  ...
  Write-Host 'Running Marvin setup and artifact generation...' -ForegroundColor Cyan
  & pwsh @setupArgs
  if ($LASTEXITCODE -ne 0) {
