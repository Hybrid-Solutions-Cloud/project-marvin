...
try {
  ...
  const secrets = new EncryptedFileStateStore(secretsPath, {}).load();
  ...
  assert.doesNotMatch(rawSecrets, /ms-secret|google-secret|apple-secret/);
  ...
  assert.match(output, /Use the Calendars page to add each account, complete provider authorization, and verify every calendar is Ready\./);
  assert.match(output, /Review privacy rules, then start synchronization from the Dashboard\./);
