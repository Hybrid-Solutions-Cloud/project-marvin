...
const tokenServer = http.createServer((req, res) => {
  if (req.method === "GET" && req.url?.startsWith("/graph/me?")) {
    ...
    res.end(JSON.stringify({ id: "ms-batch-user", displayName: "Batch User", mail: "work@example.com", userPrincipalName: "work@example.com" }));
    return;
  }
  if (req.method === "GET" && req.url?.startsWith("/graph/me/calendars")) {
    res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
    res.end(JSON.stringify({ value: [{ id: "batch-calendar", name: "Work", canEdit: true, isDefaultCalendar: true }] }));
    return;
  }
...
const server = spawn(process.execPath, ["scripts/marvin-onboard-ui.mjs"], {
  ...
  env: {
    ...
    MARVIN_DEV_AUTH_EMAIL: "marvin-batch@example.com",
    MARVIN_DEV_AUTH_DISPLAY_NAME: "Marvin Batch"
  },
  stdio: "ignore"
});
