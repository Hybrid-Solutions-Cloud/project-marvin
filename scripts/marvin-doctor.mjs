...
import { loadProviderSecretsForProfile } from "../solutions/marvin-engine/src/util/provider-secrets.mjs";
...
function buildVerificationGuidance() {
  return {
    ...
    localDevelopmentCommand: "$env:MARVIN_DEV_AUTH_ENABLED='true'; npm run marvin:ui",
...
function buildNextSteps(profile, config, connectionSummary, runtimeStatus, runtimeProcess, providerSecrets) {
  ...
  if (!hasProfile && !hasConfig) {
    ...
    steps.push("Set MARVIN_DEV_AUTH_ENABLED=true and run npm run marvin:ui, then select Local development sign-in.");
  ...
  if ((connectionSummary?.summary?.connected || 0) < (connectionSummary?.summary?.total || 0)) {
    steps.push("Use the Calendars page to authenticate or validate every calendar you expect to sync.");
  }
...
function buildDoctorReport(args) {
  ...
  const providerSecrets = loadProviderSecretsForProfile(args.rootDir, activeProfile.profileName);
